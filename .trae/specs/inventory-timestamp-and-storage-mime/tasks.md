...
- [ ] **Task 1: Switch the inventory list to `updated_at`.**
...
- [ ] **Task 2: Verify the list re-renders the right value.**
...
- [ ] **Task 3: Open the documents storage bucket to any mime type.**
  - SubTask 3.1: Create [supabase/migrations/20260101000020_open_documents_bucket.sql](file:///workspace/supabase/migrations/20260101000020_open_documents_bucket.sql) that runs:
    ```sql
...
- [ ] **Task 4: Make the upload modal show real errors.**
...
- [ ] **Task 6: Manual verification in the deployed app.**
...
- [ ] **Task 7: Commit + push + deploy.**
  - `git commit` with the migration + the hook/table changes + any UI cleanup, then `git push origin main`. Vercel auto-deploys.
